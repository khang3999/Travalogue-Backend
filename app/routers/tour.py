...
@router.post("/related") # Khi gởi qua body phải đặt tên biết giống với trong schema BehaviorLocations
async def get_related_tours(data: BehaviorLocations):
    return tour_service.get_related_tours(data.locations)

@router.get("/related1")
async def get_sorted_tours(behavior_locations: str = Query(...)):
